...
const initcode =
  "0x6394198df1600052600060006004601c335afa80601b57600080fd5b3d600060203e6040516060f3";
function calculateAddress(creatorAddress, salt, initCode) {
  const initCodeHash = utils.keccak256(initCode);
  return utils.getAddress(
    utils.hexDataSlice(
      utils.keccak256(
        utils.concat(["0xff", creatorAddress, salt, initCodeHash])
      ),
      12
    )
  );
...
contract("Springboard", accounts => {
  ...
  it("Upgrade wallet v1 to v2 should work", async () => {
    ...
    const expectedAddress = calculateAddress(
      springboard.address,
      salt,
      initcode
    );
